...
#[derive(Clone)]
struct Grid {
    // cells: BitVec<u64, Msb0>,
    cells: [bool; WIDTH * HEIGHT * 2],
...
impl Grid {
    ...
    fn insert(&mut self, point: Point) {
        self.cells[Self::to_index(point)] = true;
        // self.cells.set(Self::to_index(point), true);
    }
}

fn drop(grid: &mut Grid, r: usize, c: isize) -> usize {
    let mut n = 0;

    if grid.contains(&(r, c)) {
        return n;
    } else if r == grid.r_max + 1 {
        grid.insert((r, c));
        return n + 1;
    } else {
        n += drop(grid, r + 1, c);
